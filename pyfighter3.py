...
if "windowSurface" not in globals().keys():
...
while True:
	...
	#Draw Player
	if player['status'] == "normal":
		pygame.gfxdraw.trigon(windowSurface, player['rect'].left, player['rect'].bottom, player['rect'].right, player['rect'].bottom, player['rect'].left + int(player['rect'].width/2), player['rect'].top, WHITE)
		...
		pygame.gfxdraw.trigon(windowSurface, player['rect'].left, player['rect'].bottom, player['rect'].right, player['rect'].bottom, player['rect'].left + int(player['rect'].width/2), player['rect'].top, (rise_from_dead_count,rise_from_dead_count,rise_from_dead_count))
	...
	for wa in shock_waves:
		if wa[4] > 7:
			wa[4] -= 3
		pygame.draw.circle(windowSurface, (25*wa[3],25*wa[3],25*wa[3]), (wa[0],wa[1]), wa[2], int(wa[4]/5))
	...
	for pe in partikel_effekts:
		...
		for par in pe[1]:
			...
			pygame.gfxdraw.pixel(windowSurface, int(par[0]), int(par[1]), (par[2],par[2],par[2]))
		if pe[4] > 0:
			pe[4] -= 1
		else:
